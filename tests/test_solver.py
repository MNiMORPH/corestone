...
def test_the_base_conductance_reaches_the_matrix_through_the_triplets():
    ...
    net = m.network
    # k_matrix_at_T, not K_sat_matrix: the conductivities carry the viscosity at
    ...
    # comparison wrong by K_sat_matrix - k_matrix_at_T, which is 9.45e-11 -- and
    # np.allclose's default atol of 1e-8 swallowed it whole.
    kv = np.where(net.link_v, m.K_sat_fracture, m.k_matrix_at_T)
    kh = np.where(net.link_h, m.K_sat_fracture, m.k_matrix_at_T)
    links = kv[-1, :].copy()                       # from the row above
    links[:-1] += kh[-1, :]
    links[1:] += kh[-1, :]
    if net.periodic_x:
        kw = np.where(net.link_wrap, m.K_sat_fracture, m.k_matrix_at_T)
        links[-1] += kw[-1]
        links[0] += kw[-1]
